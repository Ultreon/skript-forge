...
public class Variable<T> implements Expression<T> {
		...
		if (isLocal && vs.isSimple()) { // Only variable names we fully know already
			...
			if (hint != null && !hint.equals(Object.class)) { // Type hint available
				...
				for (Class<? extends T> type : types) {
					...
					// Special cases
					if (type.isAssignableFrom(ServerLevel.class) && hint.isAssignableFrom(String.class)) {
						// String->World conversion is weird spaghetti code
						return new Variable<>(vs, types, isLocal, isPlural, null);
					} else if (type.isAssignableFrom(ServerPlayer.class) && hint.isAssignableFrom(String.class)) {
	...
	@Nullable Object convertIfOldPlayer(String key, Event event, @Nullable Object t){
		if(SkriptConfig.enablePlayerVariableFix.value() && t != null && t instanceof ServerPlayer){
			ServerPlayer p = (ServerPlayer) t;
			if(!p.isValid() && p.isOnline()){
				ServerPlayer player = Bukkit.getPlayer(p.getUniqueId());
