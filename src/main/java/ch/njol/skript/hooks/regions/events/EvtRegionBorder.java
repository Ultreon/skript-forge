...
import net.minecraftforge.eventbus.api.EventException;
...
public class EvtRegionBorder extends SelfRegisteringSkriptEvent {
	static {
		...
		EventValues.registerEventValue(RegionBorderEvent.class, ServerPlayer.class, new Getter<ServerPlayer, RegionBorderEvent>() {
			@Override
			public ServerPlayer get(final RegionBorderEvent e) {
				return e.getPlayer();
			}
		}, 0);
	}
	...
	static void callEvent(final Region r, final PlayerMoveEvent me, final boolean enter) {
		final ServerPlayer p = me.getPlayer();
		assert p != null;
